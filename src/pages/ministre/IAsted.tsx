...
import { DocumentList, InlineDocumentPreview } from "@/components/ministre/GeneratedDocument";
    ...
    const loadIAstedVoice = async () => {
      try {
        ...
        if (!error && voicesData?.voices) {
          const iastedVoice = voicesData.voices.find((voice: any) => voice.name.toLowerCase() === "iasted");
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground mt-1">Assistant IA Vocal avec Génération de Documents</p>
              </div>
            </div>
                ...
                <CardContent className="p-8 flex flex-col items-center justify-center min-h-[500px]">
                  <button
                    ...
                    className={`relative w-32 h-32 rounded-full transition-all duration-300 ${
                      isListening ? "neu-pressed scale-95" : "neu-raised hover:scale-105"
                  ...
                  <div className="mt-8 text-center w-full max-w-md">
                    ...
                    {isListening && silenceTimeRemaining > 0 && (
                      <div className="mt-3 space-y-2">
                        <Progress value={(silenceTimeRemaining / voiceSettings.silenceDuration) * 100} className="h-1" />
                        <p className="text-xs text-muted-foreground">Fin dans {Math.ceil(silenceTimeRemaining / 1000)}s</p>
                      </div>
                    )}
                  </div>
                  ...
                  {continuousMode && (
                    <div className="mt-6">
                      <Button onClick={toggleContinuousPause} variant={continuousModePaused ? "outline" : "default"} size="sm">
                    ...
                    {messages.length === 0 ? (
                      ...
                      messages.map((msg, idx) => (
                        <div
                          key={idx}
                          className={`${
                            msg.role === "user" ? "ml-12 neu-card-sm bg-primary/5" : "mr-12 neu-inset"
                          } p-4 rounded-lg`}
                        >
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Tous les documents créés par iAsted</p>
                  </div>
                ...
                <DocumentList documents={generatedDocuments} emptyMessage="Demandez à iAsted de créer un décret, une lettre ou un rapport" />
              </CardContent>
            </Card>
          </TabsContent>
                  ...
                  {messages.map((msg, idx) => (
                    <div key={idx} className="neu-inset p-4 rounded-lg">
                      <div className="flex items-start justify-between mb-2">
                        <span className={`text-sm font-medium ${msg.role === "user" ? "text-primary" : "text-secondary"}`}>
                          {msg.role === "user" ? "Vous" : "iAsted"}
                        </span>
                        <span className="text-xs text-muted-foreground">{new Date(msg.timestamp).toLocaleString("fr-FR")}</span>
                      </div>
              ...
              <Card className="neu-card">
                <CardContent className="p-6">
                  <div className="mb-6">
                    ...
                    <p className="text-muted-foreground">Voix officielle spécialement créée pour l'assistant ministériel</p>
                  </div>

                  <VoiceSelector selectedVoiceId={voiceSettings.voiceId} onVoiceSelect={(voiceId) => setVoiceSettings((prev) => ({ ...prev, voiceId }))} />
                </CardContent>
              </Card>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <button
                        key={model.value}
                        onClick={() => setVoiceSettings((prev) => ({ ...prev, aiModel: model.value as VoiceSettings["aiModel"] }))}
                        className={`neu-card p-4 text-center transition-all hover:scale-105 relative ${
                          voiceSettings.aiModel === model.value ? "ring-2 ring-primary bg-primary/10" : ""
                        }`}
                      >
                        {model.recommended && (
                          <div className="absolute -top-2 -right-2 px-2 py-0.5 bg-primary text-primary-foreground text-xs rounded-full">Recommandé</div>
                        )}
                ...
                <CardContent className="p-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setVoiceSettings((prev) => ({ ...prev, silenceDuration: parseInt(e.target.value, 10) }))}
                  ...
                  <div className="neu-inset p-4 rounded-lg mt-6">
                    <label className="flex items-start gap-3 cursor-pointer">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">iAsted enchaîne automatiquement les échanges</p>
                      </div>
                    </label>
                  </div>
                </CardContent>
