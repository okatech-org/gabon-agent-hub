...
export const useVoiceInteraction = () => {
  ...
  const handleInteraction = async () => {
    ...
    if (voiceState === 'idle') {
      ...
      if (continuousMode && !continuousModeToastShownRef.current) {
        unifiedToast.info("Mode conversation continue activé - iAsted écoutera automatiquement après chaque réponse");
  ...
  const toggleContinuousPause = () => {
    setContinuousModePaused(prev => {
      ...
      if (newPaused) {
        unifiedToast.info("Mode continu en pause - L'écoute automatique est suspendue");
      } else {
        unifiedToast.success("Mode continu repris - L'écoute automatique va reprendre");
      }
      return newPaused;
    });
  };
