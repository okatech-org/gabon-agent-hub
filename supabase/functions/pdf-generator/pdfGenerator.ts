 ...
 * Utilise pdfkit pour Deno: https://deno.land/x/pdfkit
 */
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .fontSize(10)
        .text('Unité – Travail – Justice', { align: 'center' })
        .moveDown(0.5);

      // Ligne de séparation
      doc
        .moveTo(fullStyle.marginLeft, doc.y)
        .lineTo(doc.page.width - fullStyle.marginRight, doc.y)
        .stroke()
        .moveDown();
      ...
      if (metadata.type === 'decree') {
        ...
        if (metadata.reference) {
          doc
            .fontSize(10)
            .text(`ARRÊTÉ N° ${metadata.reference} du ${metadata.date}`, { align: 'center' })
            .moveDown(1.5);
        }
        ...
        if (metadata.reference) {
          doc
            .text(`N° ${metadata.reference}`, { align: 'left' })
            .moveDown(1.5);
        }
      }
      ...
      // Parser et formater le contenu markdown
      ...
      for (let i = range.start; i < range.start + range.count; i++) {
        ...
        doc
          ...
          .text(
            ...
            fullStyle.marginLeft,
            doc.page.height - fullStyle.marginBottom + 20,
            { align: 'center' }
          );
      }
...
/**
 * Helper pour générer un PDF simple depuis du texte brut
 */
